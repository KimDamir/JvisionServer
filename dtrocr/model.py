...
class DTrOCRModel(nn.Module):
    ...
    def initialise_weights(self, config: DTrOCRConfig) -> None:
        ...
        pretrained_gpt = GPT2Model.from_pretrained(config.gpt_hf_model)

        # copy hidden layer weights
        for hidden_layer, pretrained_hidden_layer in zip(self.hidden_layers, pretrained_gpt.h):
            hidden_layer.load_state_dict(pretrained_hidden_layer.state_dict())

        # token embeddings
        self.token_embedding.load_state_dict(pretrained_gpt.wte.state_dict())
